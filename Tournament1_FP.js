...
const tauAtE700 = BigNumber.from('1.0001e210');
const timeLimit = 60 * 30; // 30 minutes

var tauMultiplier = 4;
var timerStarted = false;
var timer = 0;
var cumul_rho = BigNumber.ZERO;
...
var init = () => {
  ...
  /////////////////////
  // Permanent Upgrades
  theory.createPublicationUpgrade(0, currency, 1e12).level = 1;
  theory.createBuyAllUpgrade(1, currency, 1e14).level = 1;
  theory.createAutoBuyerUpgrade(2, currency, 1e17).level = 1;
  // Fractal Toggle
  {
  ...
  }
  {
    ...
    nboost.canBeRefunded = (_) => timer < timeLimit && snexp.level === 0;
  }
  {
    ...
    snexp.canBeRefunded = () => timer < timeLimit && snboost.level === 0;
  }
  {
    ...
    snboost.canBeRefunded = (_) => timer < timeLimit && sterm.level === 0;
  }
  {
    ...
    sterm.canBeRefunded = (_) => timer < timeLimit && expterm.level === 0;
  }
  {
    ...
    expterm.canBeRefunded = (_) => timer < timeLimit;
  }
...
var tick = (elapsedTime, multiplier) => {
  ...
  if (timerStarted) {
    timer += elapsedTime;
    timer = Math.min(timer, timeLimit);
  }
  ...
  if (timer < timeLimit) t_cumulative += getTdot(tvar.level) * dt;
  ...
  if (timer < timeLimit) q += fractalTerm.level > 0 ? qdot * dt : 0;
  ...
  if (timer < timeLimit) r += fractalTerm.level > 1 ? rdot * dt : 0;
...
var getInternalState = () => `${q} ${r} ${t_cumulative} ${timerStarted} ${timer.toString()} ${cumul_rho.toString(4)}`;
...
var setInternalState = (state) => {
  ...
  if (values.length > 3) timerStarted = values[3].toLowerCase() === 'true';
  if (values.length > 4) timer = parseFloat(values[4]);
  if (values.length > 5) cumul_rho = parseBigNumber(values[5]);
...
var getPrimaryEquation = () => {
  if (stage === 0) {
    theory.primaryEquationHeight = 155;
    ...
    if (fractalTerm.level > 1) {
      result += snboost.level === 0 ? ", \\qquad S_n = 3^{n-1}" : ", \\qquad S_n = 2(3^{n-1}) - 1";
      result += "\\\\";
    }
    result += `\\text{Timer: ${getTimeString(timer)}/${getTimeString(timeLimit)}} \\\\`;
    result += `\\text{Cumulative \\rho: ${cumul_rho.toString(4)}}`;
    return result;
  } else {
...
var getTau = () => currency.value.pow(0.075*tauMultiplier).max(tauAtE700);
